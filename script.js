...
const toggleAddTaskForm = () => {
  ...
  // Reset form if closing without saving while editing
  if (!addTaskWrapper.classList.contains("active") && isEditing) {
    resetForm();
  }
};
...
const checkTaskAlarms = () => {
  const now = new Date();
  // Filter tasks that are due, not completed and not yet alarmed (and have a priority)
  const dueTasks = tasks.filter(task => {
    if (task.date && task.time && !task.alarmed && !task.completed && task.priority) {
  ...
  // Define priority order: lower value means higher priority
  const priorityOrder = {
    high: 1,
    medium: 2,
    low: 3,
  ...
  // Sort tasks due at the same date & time by their priority
  dueTasks.sort((a, b) => {
  ...
  // Notify each task in sorted order without showing the priority in the alert
...
const timeInput = document.getElementById("time-input");      // added
